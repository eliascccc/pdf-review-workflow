...
def run_dev_single_file(file_name: str = "PO 9913782.eml") -> None:
    """
    DEV helper:
    Copy one test file into uploads and run the normal pipeline.
    Works with .pdf, .msg, or .eml.
    """
    source_path = BASE_DIR / file_name

    if not source_path.exists():
        raise FileNotFoundError(f"Could not find file: {source_path}")

    # Clear uploads for a clean test
    for file in UPLOADS_DIR.iterdir():
        if file.is_file():
            file.unlink()

    shutil.copy(source_path, UPLOADS_DIR / source_path.name)
...
if __name__ == "__main__":
    run_worker_forever()
    # run_dev_single_file("PO 9913782.eml")
   # run_dev_single_file("PO 9913782.pdf")
    # run_dev_single_file("some_mail.msg")